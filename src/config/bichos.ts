...
export function calcularGrupo(milhar: string): number {
  const dezena = parseInt(milhar.slice(-2), 10);
  if (dezena === 0) return 25;
  return Math.ceil(dezena / 4);
}

export function calcularBicho(milhar: string): string {
  const grupo = calcularGrupo(milhar);
  return BICHOS[grupo - 1].nome;
}
